...
class FeatureFusion(tf.keras.Model):
    def __init__(self, units):
        ...
        self.avg_pool3d = tf.keras.layers.GlobalAvgPool3D()
        self.fc1 = tf.keras.layers.Dense(units=units, use_bias=True,
                                      kernel_initializer='he_normal',
                                      kernel_regularizer=tf.keras.regularizers.l2(L2))
        self.relu = tf.keras.layers.Activation(activation='relu')
        self.fc2 = tf.keras.layers.Dense(units=units, use_bias=True,
                                      kernel_initializer='he_normal',
                                      kernel_regularizer=tf.keras.regularizers.l2(L2))
        self.sigmoid = tf.keras.layers.Activation(activation='sigmoid')
        self.multiply1 = tf.keras.layers.Multiply()
        self.multiply2 = tf.keras.layers.Multiply()
