...
class Refinement2(keras.Model):
    def __init__(self, filters):
        ...
        self.conv5 = conv_bn_act(filters, 3, 1, 'same', 1)
        self.conv6 = conv2d(1, 3, 1, 'same', 1)
        self.upsample = tf.keras.layers.UpSampling3D(size=(4, 4, 1))

    def call(self, inputs, training=None, mask=None):
        # inputs: [disparity_rf, disparity0, rgb, gx, gy]
        assert len(inputs) == 5

        scale_factor = inputs[2].shape[1] / inputs[0].shape[1]
        disp = self.upsample(inputs[0])
        disp_sq = tf.squeeze(disp, [-1])
        disp = disp_sq * scale_factor
        
        disp2 = tf.image.resize(inputs[1], [inputs[2].shape[1], inputs[2].shape[2]])
        disp2 = disp2 * scale_factor

        concat = tf.concat([disp, inputs[2], inputs[3], inputs[4]], -1)
        delta = self.conv1(concat)
        delta = self.conv2(delta)
        delta = self.conv3(delta)
        delta = self.conv4(delta)
